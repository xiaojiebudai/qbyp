...
public class AddBankActivity extends FatherActivity implements OnClickListener {
	...
	private void commit() {
		...
		if (button.getText().toString().trim().equals(getString(R.string.deposit_card))) {
			bank.CardType = 0;
		} else {
			bank.CardType = 1;
		}
						...
						if (data.getIntValue("ErrCode") == 104) {
							final CommonDialog commonDialog = DialogUtils
									.getCommonDialog(AddBankActivity.this,
											R.string.no_auth_cannot_add_bank);
							commonDialog.getButtonRight(R.string.go_to_auth);
							commonDialog.getButtonLeft(R.string.temp_no);
